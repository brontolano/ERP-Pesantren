...
class WaliController extends Controller
{
    ...
    public function tagihan(Request $request, int $studentId): JsonResponse
    {
        ...
        $invoices = Invoice::where('student_id_fk', $studentId)
            ->whereNotIn('status', ['draft', 'cancelled'])
            ->with('items:id,invoice_id,description,amount')
            ->orderByDesc('created_at')
            ->get()
            ->map(function ($inv) {
                $sisa = max(0, (float) $inv->total_amount - (float) $inv->paid_amount);

                return [
                    'id'             => $inv->id,
                    'nomor'          => $inv->invoice_number,
                    'jenis'          => $inv->invoice_type,
                    'keterangan'     => $inv->hijri_label ?: $this->invoiceTypeLabel($inv->invoice_type),
                    'label_periode'  => $inv->hijri_label,
                    'is_prioritas'   => $this->isPriorityType($inv->invoice_type),
                    'total'          => (float) $inv->total_amount,
                    'terbayar'       => (float) $inv->paid_amount,
                    'sisa'           => $sisa,
                    'status'         => $this->normalizeInvoiceStatus($inv->status),
                    'status_raw'     => $inv->status,
                    'due_date'       => $inv->due_date?->format('Y-m-d'),
                    'jatuh_tempo'    => $inv->due_date?->format('Y-m-d'),
                    'items'          => $inv->items->map(fn($i) => [
                        'id'         => $i->id,
                        'nama_item'  => $i->description,
        ...
        $unpaid = $invoices->whereIn('status_raw', ['issued', 'partial', 'overdue']);
        $total_tunggakan     = (float) $unpaid->sum('sisa');
        $tunggakan_prioritas = (float) $unpaid->where('is_prioritas', true)->sum('sisa');

        return response()->json([
            'ok'                  => true,
            'total_tunggakan'     => $total_tunggakan,
            'tunggakan_prioritas' => $tunggakan_prioritas,
            'invoices'            => $invoices->values(),
            'data'                => $invoices->values(),
        ]);
    }

    /** Jenis tagihan prioritas yang harus dilunasi sebelum wali bisa topup saldo jajan. */
    private function isPriorityType(?string $type): bool
    {
        return in_array($type, ['spp', 'ujian', 'daftar_ulang'], true);
    }
    ...
    private function normalizeInvoiceStatus(string $status): string
    {
        return match ($status) {
            'paid'    => 'paid',
            'partial' => 'partial',
            default   => 'unpaid', // issued, overdue, dll
        };
    }

    /**
     * Hitung total tunggakan prioritas (SPP/Ujian/Daftar Ulang) yang belum lunas.
     */
    private function priorityTunggakan(int $studentId): float
    {
        return (float) Invoice::where('student_id_fk', $studentId)
            ->whereIn('invoice_type', ['spp', 'ujian', 'daftar_ulang'])
            ->whereIn('status', ['issued', 'partial', 'overdue'])
            ->get()
            ->sum(fn($inv) => max(0, (float) $inv->total_amount - (float) $inv->paid_amount));
    }
    ...
    public function tabungan(Request $request, int $studentId): JsonResponse
    {
        ...
        $tunggakan_prioritas = $this->priorityTunggakan($studentId);
        $override = $savings ? (bool) ($savings->allow_topup_override ?? false) : false;
        $topup_jajan_locked = $tunggakan_prioritas > 0 && !$override;
        ...
        if (!$savings) {
            return response()->json([
                ...
                'data' => [
                    ...
                    'tunggakan_prioritas'  => $tunggakan_prioritas,
                    'topup_jajan_locked'   => $topup_jajan_locked,
    ...
    public function freezeTabungan(Request $request, int $studentId): JsonResponse
    {
        ...
        // Terima 'freeze' (kontrak lama) maupun 'frozen' (dipakai PWA).
        $freeze = $request->has('freeze') ? $request->boolean('freeze') : $request->boolean('frozen');

        DB::table('student_savings')->updateOrInsert(
            ['student_id' => $studentId],
            ['is_frozen' => $freeze, 'updated_at' => now()]
        );

        $status = $freeze ? 'dibekukan' : 'diaktifkan kembali';

        return response()->json([
            'ok'      => true,
            'message' => "Tabungan berhasil {$status}.",
            'data'    => ['is_frozen' => $freeze],
        ]);
    }

    /**
     * POST /api/v1/wali/santri/{studentId}/tabungan/bayar-tagihan
     * Bayar tagihan/tunggakan langsung dari saldo tabungan santri (LANGSUNG LUNAS).
     * Mendebit saldo, mencatat Payment (invoice otomatis ter-rekalkulasi), dan
     * menulis savings_transactions berjenis debit.
     */
    public function bayarTagihanDariSaldo(Request $request, int $studentId): JsonResponse
    {
        $this->authorizeStudent($request, $studentId);
        ...
        $sisa = max(0, (float) $invoice->total_amount - (float) $invoice->paid_amount);
        if ($sisa <= 0) {
            return response()->json(['ok' => false, 'message' => 'Tagihan ini sudah lunas.'], 422);
        }

        // Nominal default = lunasi penuh. Jika dikirim, batasi maksimal = sisa.
        $nominal = $request->filled('nominal') ? min((float) $request->nominal, $sisa) : $sisa;

        try {
            $result = DB::transaction(function () use ($studentId, $invoice, $nominal, $request) {
                ...
                if (!$savings) {
                    abort(422, 'Saldo tabungan belum tersedia. Lakukan setor saldo terlebih dahulu.');
                }
                if ($savings->is_frozen) {
                    abort(422, 'Tabungan sedang dibekukan. Aktifkan kembali untuk membayar.');
                }
                if ((float) $savings->saldo < $nominal) {
                    abort(422, 'Saldo tabungan tidak cukup. Saldo: Rp '
                        . number_format((float) $savings->saldo, 0, ',', '.')
                ...
                DB::table('student_savings')
                    ->where('student_id', $studentId)
                    ->update(['saldo' => $saldoSesudah, 'last_transaction_at' => now(), 'updated_at' => now()]);

                // Catat pembayaran -> model Payment otomatis rekalkulasi invoice (auto lunas/partial).
                \App\Models\Payment::create([
                    ...
                    'payment_channel' => 'saldo_santri',
                    'reference_number'=> 'SALDO-' . $invoice->invoice_number,
                    'received_by'     => $request->user()->id,
                    'notes'           => 'Pembayaran dari saldo tabungan santri',
                    'verified_at'     => now(),
                ...
                DB::table('savings_transactions')->insert([
                    'student_id'    => $studentId,
                    'jenis'         => 'debit',
                    'kategori'      => 'bayar_tagihan',
                    'nominal'       => $nominal,
                    'saldo_sesudah' => $saldoSesudah,
                    'keterangan'    => 'Bayar tagihan ' . $invoice->invoice_number,
            ...
            return response()->json(['ok' => false, 'message' => $e->getMessage()], $e->getStatusCode() ?: 422);
        }

        return response()->json([
            'ok'      => true,
            'message' => $result['invoice_status'] === 'paid'
                ? 'Tagihan berhasil dilunasi dari saldo tabungan.'
                : 'Pembayaran sebagian berhasil dari saldo tabungan.',
            'data'    => [
    ...
    public function topupTabungan(Request $request, int $studentId): JsonResponse
    {
        ...
        // Gating prioritas: SPP/Ujian harus lunas sebelum wali bisa setor saldo jajan,
        // kecuali staff pesantren memberi pengecualian (allow_topup_override).
        $savings = DB::table('student_savings')->where('student_id', $studentId)->first();
        $override = $savings ? (bool) ($savings->allow_topup_override ?? false) : false;
        $tunggakan_prioritas = $this->priorityTunggakan($studentId);

        if ($tunggakan_prioritas > 0 && !$override) {
            return response()->json([
                'ok'                  => false,
                'code'                => 'PRIORITY_BILLS_UNPAID',
                'message'             => 'Lunasi tagihan prioritas (SPP/Ujian) sebesar Rp '
                    . number_format($tunggakan_prioritas, 0, ',', '.')
                    . ' terlebih dahulu. Hubungi staff pesantren bila perlu pengecualian.',
                'tunggakan_prioritas' => $tunggakan_prioritas,
            ], 422);
        }
