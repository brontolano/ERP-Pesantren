...
class ImportExportSantri extends Page
{
    ...
    public function exportExcel(): \Symfony\Component\HttpFoundation\BinaryFileResponse
    {
        ...
        $headers = ['NIS', 'NISN', 'NIK', 'Nama Lengkap', 'L/P', 'Tempat Lahir', 'Tgl Lahir', 'Kelas', 'Kelas Detail', 'Jenjang', 'Tahun Masuk', 'Status', 'Jalur Masuk', 'SPP', 'Tunggakan (Bln)', 'No KK', 'Ayah', 'HP Ayah', 'Ibu', 'HP Ibu', 'Alamat', 'Desa/Kelurahan', 'Kecamatan', 'Kab/Kota', 'Provinsi', 'Kode Pos', 'Kode Wilayah'];
        ...
        foreach ($students as $s) {
            ...
            $sheet->fromArray([
                ...
                $village?->name ?? $s->desa_kelurahan,
                $village?->district?->name ?? $s->kecamatan,
                $village?->district?->city?->name ?? $s->kab_kota,
                $village?->district?->city?->province?->name ?? $s->provinsi,
                $village?->postal_code ?? $s->kode_pos,
                $s->village_code,
            ], null, "A{$row}");
            $row++;
        }
