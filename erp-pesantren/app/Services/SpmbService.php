...
class SpmbService
{
    ...
    {
        $existingInvoice = Invoice::where('ppdb_registration_id', $registration->id)
            ->where('invoice_type', 'daftar_ulang')
            ->first();
    ...
    public function convertToSantri(PpdbRegistration $registration): void
    {
        ...
        $student = Student::where('ppdb_registration_id', $registration->id)
            ->orWhere('enrolled_from_ppdb_id', $registration->id)
            ->first();
        ...
        if (!$student) {
            ...
            $student = Student::create([
                ...
                'alamat' => $registration->alamat_jalan,
                'rt_rw' => trim(($registration->rt ?? '') . '/' . ($registration->rw ?? ''), '/'),
                'province_code' => $registration->province_code,
                'city_code' => $registration->city_code,
                'district_code' => $registration->district_code,
                'village_code' => $registration->village_code,
            ...
            $student->update([
                'erp_account_id' => $student->erp_account_id ?: $account?->id,
                'wali_account_id' => $student->wali_account_id ?: $account?->id,
                'status' => 'aktif',
                'ppdb_registration_id' => $student->ppdb_registration_id ?: $registration->id,
                'enrolled_from_ppdb_id' => $student->enrolled_from_ppdb_id ?: $registration->id,
            ]);
        }
