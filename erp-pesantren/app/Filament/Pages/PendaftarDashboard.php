...
class PendaftarDashboard extends Page
{
    ...
    public function mount(): void
    {
        ...
        $this->mandatoryDocs = config('spmb.mandatory_documents', []);
        ...
        $this->registrations = $user?->registrations()
            ...
            ->map(function (PpdbRegistration $reg) {
                ...
                $uploadedTypes = $reg->documents->pluck('document_type')->unique()->values()->all();
                $reg->missing_docs = collect($this->mandatoryDocs)
                    ->filter(fn ($label, $type) => !in_array($type, $uploadedTypes, true))
                    ->values()
                    ->all();
